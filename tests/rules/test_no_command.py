...
import pytest
from thefail.rules.no_command import match, get_new_command
...
def bins_exists(request):
    p = patch('thefail.rules.no_command.which',
              return_value=True)
    p.start()
...
def test_match(command_found, command_not_found, settings):
    with patch('thefail.rules.no_command.Popen') as Popen:
...
def test_get_new_command(command_found):
    with patch('thefail.rules.no_command._get_output',
